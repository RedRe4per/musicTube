...
export const TrackPlay = ({ trackId, album, duration }: Props) => {
  ...
  useEffect(()=>{
    const getTrackIndex = async () => {
        ...
        const index =  albumData.songs.findIndex((song: IAlbumSong)=>song.id === trackId)
        setSongIndex(index);
    ...
    <section className="mt-6 ml-10 flex gap-10 items-center justify-between">
      ...
      <button className="hover:animate-pulse">
        <Image
          ...
          className={currentMusic?.id !== trackId || isMusicPlay ? "" : "hidden"}
      ...
      <svg className="w-[48px] h-[48px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="none" d="M0 0h24v24H0z"/><path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm0-2a8 8 0 1 0 0-16 8 8 0 0 0 0 16zm1-8h4v2h-6V7h2v5z" fill="#F1F1F1"/></svg>
        <span className="mt-1 text-h3-normal text-gray-200">{formatTime(duration / 1000)}</span>
      </section>
    </section>
  );
};
