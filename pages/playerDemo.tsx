...
export default function AudioPlayerDemo(props: Props) {
    ...
    const handleRandomMusic = () => {
        ...
        if (musicArray.length > 1) {
            ...
            while (randomMusicIndex === currentMusicIndex || randomMusicIndex === -1) {
    ...
    return (
        <main>
            ...
            <p>{ }</p>
            <audio ref={musicPlayers} src={currentMusic.url} loop={isMusicLoop} onEnded={handleMusicEnded} />
            <div className="flex gap-5">
                <button onClick={handlePlayAndPause}>{isMusicPlay ? 'Play' : 'Pause'}</button>
                <button onClick={handleLastMusic}>Last</button>
                <button onClick={handleNextMusic}>Next</button>
                <button onClick={handleLoop} className={isMusicLoop ? "bg-green-500" : "bg-slate-500"}>Loop</button>
                <button onClick={handleRandomPlay} className={isRandomPlay ? "bg-green-500" : "bg-slate-500"}>Random</button>
            </div>
        </main>
    )
}
...
export async function getServerSideProps() {
    const res = await fetch(`https://music-server-sand.vercel.app/song/url/v1?id=405998841,33894312,298317,1888354230,210049&level=lossless`);
